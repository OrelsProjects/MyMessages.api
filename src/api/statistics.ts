...
export const getCallsCountByDay = async (req, context) =>
  runRequest(req, context, async (req, user_id) => {
    ...
    const query =
      "select  EXTRACT(DAY FROM start_date) as day,\n" +
...
export const getCallsCount = async (req, context) =>
  runRequest(req, context, async (req, user_id) => {
    let start_date = null;
    let end_date = null;
    if (req.queryStringParameters) {
...
export const getMessagesSentCount = async (req, context) =>
  runRequest(req, context, async (req, user_id) => {
    let start_date = null;
    let end_date = null;
    if (req.queryStringParameters) {
    ...
    const query =
      "select Count( m_ms.title), m_ms.title\n" +
      "from (\n" +
      "messages_sent\n" +
      "as ms join messages\n" +
